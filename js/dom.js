...
+function(cal,$){
	...
	function calendarHTML(y,m){
		...
		// HTML fragment of calendar body
		var html='';	
	...
	function detailHTML(y,m,d){
		...
		dtmpl+='<span><%=lunarMonth%><%=lunarDay%></span><span><%=eraYear%>【<%=zodiac%>】</span><span><%=eraMonth%><%=eraDay%></span> </p>';
		dtmpl+='<div class="op-calendar-new-right-almanacbox">';
		dtmpl+='<p class="op-calendar-new-right-almanac c-clearfix">';
		dtmpl+='<span class="op-calendar-new-right-suit"><i>do</i><%=yDay%></span>'; 
		dtmpl+='<span class="op-calendar-new-right-avoid"><i>dont</i><%=jDay%></span></p>';
		dtmpl+='</div>';
		...
		var yDay=selectedDay.almanac?selectedDay.almanac.y.split('.').slice(0,6).join('<br>'):'--';
		var jDay=selectedDay.almanac?selectedDay.almanac.j.split('.').slice(0,6).join('<br>'):'--';
		var day={
			today:y+'-'+m+'-'+d,
			dayTime:d,
	...
	/**
	 * Initialize year, month, holiday selection control
	 * @param {Number} y
	 * @param {Number} m
	 */
	function dropDownHTML(y,m,d){
		...
		// 输入框中的年
		$('.op-calendar-new-year-box .c-dropdown2-btn').html(y)
		$('.op-calendar-new-year-box .c-dropdown2-btn').attr('data-value',y);
		$('.op-calendar-new-year-box .c-dropdown2-menubox').html(html)
		
		var html='';
		for(var i=1;i<=12;i++){
			html+='<li class="c-dropdown2-option" data-value="'+i+'" data-role="m">'+i+'</li>';
		}
		// Month in the input box
		$('.op-calendar-new-month-box .c-dropdown2-btn').html(m);
